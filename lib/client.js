...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // host returns archive order, and every field needed to re-order a row is
    // already in it (lastActivity / createdAt / title).
    const SORT_MODES = [
    ...
    const SORT_KEY = 'dsh-archive-browser.sort';

    function loadSortMode() {
      try {
        if (typeof localStorage === 'undefined') return 'archive';
        const saved = localStorage.getItem(SORT_KEY);
        return SORT_MODES.some((mode) => mode.key === saved) ? saved : 'archive';
      } catch (error) {
        return 'archive';
      }
    }
    ...
    function cycleSort() {
      ...
      if (lastSessions !== null) {
        ...
        setStatus('共 ' + lastSessions.length + ' 个归档会话 · 顺序：' + next.label + '（' + next.hint + '）', false);
      }
    }
    ...
    async function refreshList() {
      ...
      setStatus(
        degraded
          ...
          : sessions.length
            ? '共 ' + sessions.length + ' 个归档会话 · 顺序：' + sortModeInfo().label + '（' + sortModeInfo().hint + '）'
            : '没有已归档的会话。',
        false,
      );
      busy = false;
