...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    let titleHost = null;
    let busy = false;
    // Archived-list ordering. Persisted in localStorage (best-effort) so the
    // choice survives a reload; the panel works fine when storage is absent
    // (the offline harness runs without it).
    let sortMode = loadSortMode();
    let lastSessions = null;
    let sortBtn = null;
    // The cheap list omits per-session message counts on purpose (the shell's
    // projection cache measures different things — see archive-core). Opening a
    // session fills its row in from the detail response, so a row gains its
    // counts only after you actually look at that conversation.
    const detailMeta = new Map();

    // ── list ordering ───────────────────────────────────────────────────
    // ONE cycling button in the panel header. Ordering happens client-side: the
    // list already carries every field needed to re-order a row (lastActivity /
    // createdAt / title). The DEFAULT is most-recent-first, so the freshest
    // archived conversation is at the top the moment the panel opens.
    const SORT_MODES = [
      { key: 'recent', label: '最近活动↓', hint: '最近用过的排在前面' },
      { key: 'oldest', label: '最近活动↑', hint: '最早用过的排在前面' },
      { key: 'archive', label: '归档顺序', hint: '按你归档的先后顺序排' },
      { key: 'title', label: '标题', hint: '按标题排序' },
    ];
    // v2: the key changed when the default flipped to most-recent-first, so a
    // choice saved by the previous default cannot pin the old ordering.
    const SORT_KEY = 'dsh-archive-browser.sort.v2';

    function loadSortMode() {
      try {
        if (typeof localStorage === 'undefined') return 'recent';
        const saved = localStorage.getItem(SORT_KEY);
        return SORT_MODES.some((mode) => mode.key === saved) ? saved : 'recent';
      } catch (error) {
        return 'recent';
      }
    }

    function saveSortMode(key) {
      try {
        if (typeof localStorage !== 'undefined') localStorage.setItem(SORT_KEY, key);
      } catch (error) {
        /* persistence is best-effort -- never break the panel over it */
      }
    }

    function sortModeInfo() {
      return SORT_MODES.find((mode) => mode.key === sortMode) || SORT_MODES[0];
    }

    /** A row's activity timestamp, falling back to its creation time. */
    function activityAt(session) {
    ...
    /** Order a fetched list per the active mode (archive order = untouched). */
    function orderSessions(sessions) {
      const list = sessions.slice();
      if (sortMode === 'recent') list.sort((a, b) => activityAt(b) - activityAt(a));
      else if (sortMode === 'oldest') list.sort((a, b) => activityAt(a) - activityAt(b));
    ...
    function updateSortButton() {
      if (sortBtn !== null) sortBtn.textContent = '顺序：' + sortModeInfo().label;
    }

    /** Cycle to the next ordering and re-render the list we already fetched. */
    function cycleSort() {
      const at = SORT_MODES.findIndex((mode) => mode.key === sortMode);
      const next = SORT_MODES[(at + 1) % SORT_MODES.length];
      sortMode = next.key;
      saveSortMode(sortMode);
      diag('sort -> ' + sortMode);
      updateSortButton();
      if (lastSessions !== null) {
        renderRows(orderSessions(lastSessions));
        setStatus(listStatus(lastSessions, false), false);
      }
    }
    ...
    function makeStyles(p) {
      return {
        ...
        // ── transcript: one colour per voice, so 你 / 助手 / 工具 scan apart ──
        turnCard: 'border-left:3px solid ' + p.border + ';padding:4px 0 4px 9px;margin-bottom:9px;',
        turnCardTool:
          'border-left:3px solid ' + p.border + ';background:' + p.rowBg + ';border-radius:0 6px 6px 0;' +
          'padding:4px 8px 4px 9px;margin-bottom:6px;',
        barUser: 'border-left-color:' + (p.dark ? '#7cc0ff' : '#1b5e9e') + ';',
        barAssistant: 'border-left-color:' + (p.dark ? '#6ee7a8' : '#1a7f4b') + ';',
        barTool: 'border-left-color:' + (p.dark ? '#b9bcc4' : '#5c6068') + ';',
        roleUser: 'font-size:11px;font-weight:600;color:' + (p.dark ? '#7cc0ff' : '#1b5e9e') + ';margin-bottom:2px;',
        roleAssistant: 'font-size:11px;font-weight:600;color:' + (p.dark ? '#6ee7a8' : '#1a7f4b') + ';margin-bottom:2px;',
        roleTool: 'font-size:11px;font-weight:600;color:' + (p.dark ? '#b9bcc4' : '#5c6068') + ';margin-bottom:2px;',
        toolText:
          'font-family:ui-monospace,Consolas,"Courier New",monospace;font-size:11px;color:' + p.subtle +
          ';word-break:break-word;',
        legend: 'font-size:11px;line-height:1.8;color:' + p.subtle + ';margin-bottom:8px;',
    ...
    function buildPanel() {
      ...
      sortBtn = button('顺序：' + sortModeInfo().label, () => {
        cycleSort();
      });
      sortBtn.setAttribute('title', '点一下换一种排序方式：归档顺序 → 最近活动↓ → 最近活动↑ → 标题');
    ...
    async function refreshList() {
      ...
      renderRows(orderSessions(sessions));
      setTitle(sessions.length);
      updateSortButton();
      setStatus(listStatus(sessions, degraded), false);
      busy = false;
    }
    ...
    function renderRows(sessions) {
      ...
      for (const session of sessions) {
        const known = detailMeta.get(session.id);
        listHost.appendChild(buildRow(known === undefined ? session : { ...session, ...known }));
      }
    }

    /** Keep one session's detail metadata so its row fills in without a re-fetch. */
    function rememberDetail(sessionId, meta) {
      if (meta === undefined || meta === null) return;
      detailMeta.set(sessionId, {
        userCount: meta.userCount,
        assistantCount: meta.assistantCount,
        firstMessage: typeof meta.firstMessage === 'string' && meta.firstMessage !== '' ? meta.firstMessage : undefined,
      });
    }

    /**
     * The list view's status line. Shared by the initial fetch and by re-sorting,
     * so the ordering hint AND the soft-load note stay put when the user cycles
     * the sort button (a stale copy here silently dropped the note).
     */
    function listStatus(sessions, degraded) {
      if (degraded) return '宿主接口不可用，已降级为客户端列表（查看/恢复仍需要宿主）';
      if (sessions.length === 0) return '没有已归档的会话。';
      const countsMissing = sessions.some(
        (session) => session.userCount === undefined && session.assistantCount === undefined,
      );
      return (
        '共 ' + sessions.length + ' 个归档会话 · 顺序：' + sortModeInfo().label +
        '（' + sortModeInfo().hint + '）' +
        (countsMissing ? ' · 轻量载入：点开某个会话才读它的内容' : '')
      );
    }
    ...
    async function showDetail(sessionId) {
      ...
      try {
        ...
        rememberDetail(sessionId, detail.meta);
        listHost.textContent = '';
        ...
        // Colour key, so the three voices explain themselves.
        const legend = el('div', S.legend);
        legend.appendChild(el('span', S.roleUser, '▎你（你说的）'));
        legend.appendChild(el('span', S.roleAssistant, '   ▎助手（回复）'));
        legend.appendChild(el('span', S.roleTool, '   ▎工具（它干的活）'));
        listHost.appendChild(legend);
        ...
        for (const turn of detail.turns || []) {
          ...
          const card = el(
            'div',
            isTool ? S.turnCardTool + S.barTool : S.turnCard + (isUser ? S.barUser : S.barAssistant),
          );
          const roleLabel = isTool
            ? '🔧 ' + (turn.tool || '工具') + (turn.isError === true ? '（失败）' : '')
            : isUser
              ? '你'
              : '助手';
          card.appendChild(el('div', isTool ? S.roleTool : isUser ? S.roleUser : S.roleAssistant, roleLabel));
          card.appendChild(
            isTool
              ? el('div', S.toolText, turn.detail ? String(turn.detail) : String(turn.text || ''))
              : el('div', S.turnText, turn.text),
          );
    ...
    async function showImpact(sessionId) {
      ...
      try {
        ...
        renderImpact(impact);
        rememberDetail(sessionId, impact.meta);
        const stats = impact.stats || {};
        setStatus(
