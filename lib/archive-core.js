...
/**
 * The content blocks of one message record, whichever shape the log uses.
 *
 * Shapes differ per record type (verified against real logs, 2026-09-12):
 *   user/message      -> data.content
 *   assistant/message -> data.message.content   <-- NOT data.content
 *   tool/result       -> data.message.content
 * Reading only `data.content` silently dropped EVERY assistant reply from the
 * transcript (the detail view showed just the user's messages and the tool
 * calls), so this helper is the single place that knows the difference.
 */
...
export function extractMeta(records) {
  ...
  for (const r of records) {
    ...
    if (r?.type === 'user/message') {
      userCount += 1;
      if (firstMessage === '') {
        const text = contentToText(messageContent(r));
...
/** Per-tool argument field that best says WHAT the call acted on. */
const TOOL_SUMMARY_FIELDS = {
  pwsh: ['command', 'description'],
  bash: ['command', 'description'],
  'pwsh-persistent': ['command'],
  'bash-persistent': ['command'],
  read: ['file_path'],
  write: ['file_path'],
  edit: ['file_path'],
...
/**
 * One-line "what did this call act on" summary for the transcript view.
 *
 * A raw transcript is mostly tool noise, so the detail view needs a hint of what
 * each call touched (`pwsh · Get-ChildItem …`, `read · src/app.ts`). The
 * arguments are logged losslessly, so this is pure formatting -- no guessing.
 *
 * @returns a short single-line summary, or '' when nothing useful is known.
 */
export function summarizeToolCall(name, argsJson) {
  let args;
  try {
    args = JSON.parse(typeof argsJson === 'string' ? argsJson : '{}');
  } catch {
    return '';
  }
  ...
  const describe = (value) => {
    ...
    if (Array.isArray(value)) {
      ...
      const text =
        typeof head === 'string'
          ? clean(head)
          : typeof head?.query === 'string'
            ? clean(head.query)
            : typeof head?.text === 'string'
              ? clean(head.text)
              : '';
      return text === '' ? value.length + ' 项' : value.length + ' 项 · ' + text;
    }
    return '';
  };

  for (const key of TOOL_SUMMARY_FIELDS[name] ?? []) {
    const text = describe(args[key]);
    if (text !== '') return text;
  }
  // Unknown tool (or a renamed field): still say something rather than nothing.
...
/**
 * Build a readable transcript: user / assistant text turns in order, with tool
 * calls kept as compact entries carrying the tool name, a short summary of what
 * it acted on, and whether the call failed. Roles stay distinct so the detail
 * view can colour them.
 */
export function buildTranscript(records, { maxChars = 200000 } = {}) {
  // Failed calls, so the transcript can mark them without a second pass.
  const failed = new Set();
  for (const r of records) {
    if (r?.type !== 'tool/result') continue;
    const block = Array.isArray(r?.data?.message?.content) ? r.data.message.content[0] : undefined;
    if (block?.isError !== true) continue;
    const callId = r?.data?.message?.source?.callId ?? block?.toolCallId;
    if (typeof callId === 'string') failed.add(callId);
  }

  const turns = [];
  for (const r of records) {
    if (r?.type === 'user/message') {
      const text = contentToText(messageContent(r)).trim();
      ...
      const name = String(r.data?.name ?? r.data?.toolName ?? 'tool');
      const detail = summarizeToolCall(name, r.data?.arguments);
      turns.push({
        role: 'tool',
        tool: name,
        detail,
        isError: typeof r.data?.callId === 'string' && failed.has(r.data.callId),
        text: detail === '' ? name : name + ' · ' + detail,
        time: r.time,
      });
    }
  }
...
/** One archived session's list entry (exact: decodes the transcript). */
...
/**
 * Cheap session-artifact index: `sessions/<group>/<sessionId>/session.jsonl.zstd`.
 *
 * Directory listing + stat only -- no file contents are read, so this costs a
 * handful of syscalls no matter how large the logs are.
 *
 * @returns Map sessionId -> { artifact, bytes, mtimeMs }
 */
export function collectSessionArtifacts(home = resolveDshHome()) {
  const root = path.join(home, 'sessions');
  const byId = new Map();
  ...
  for (const group of fs.readdirSync(root, { withFileTypes: true })) {
    ...
    for (const entry of fs.readdirSync(groupDir, { withFileTypes: true })) {
      ...
      try {
        ...
        /* leave them undefined; the row still renders */
      }
      byId.set(entry.name, { artifact, bytes, mtimeMs });
    }
  }
  return byId;
}
...
/**
 * Read one session's projection checkpoint.
 *
 * `<DSH_HOME>/storages/session_projcache/sessions/<sessionId>.json` is a small
 * plain-JSON snapshot the shell keeps per session (written by
 * `dsh-session-projection-cache`). It already carries everything the archived
 * list shows -- title, first prompt, prompt count, turn outline, last prompt
 * time, cwd, createdAt, agent preset -- so the list needs no transcript decode.
 *
 * @returns the checkpoint's `record`, or undefined when there is none.
 */
export function readProjectionCheckpoint(home, sessionId) {
  const file = path.join(home, 'storages', 'session_projcache', 'sessions', `${sessionId}.json`);
  try {
    return JSON.parse(fs.readFileSync(file, 'utf8'))?.record;
  } catch {
    return undefined;
  }
}

/** One projection value out of a checkpoint record. */
function projectionValue(record, key) {
  return record?.rows?.[key]?.val;
}

/**
 * One archived session's LIST entry, built cheaply.
 *
 * Everything the list renders comes from the projection checkpoint plus a stat.
 * The transcript is decoded only when a session has NO checkpoint at all (a
 * session this build never opened), so the expensive path stays rare and bounded
 * to that one row.
 *
 * A checkpoint is a periodic snapshot, so counts can lag the log slightly; the
 * detail views (`read` / `impact`) always decode the real thing.
 */
export function describeArchivedSessionLite(home, sessionId, artifacts) {
  const known = (artifacts ?? collectSessionArtifacts(home)).get(sessionId);
  if (known === undefined) {
    return { id: sessionId, missing: true, title: '(artifact missing)', recordCount: 0, lightweight: true };
  }
  const checkpoint = readProjectionCheckpoint(home, sessionId);
  if (checkpoint === undefined) {
    // No checkpoint: pay the full decode for THIS row only (correctness first).
    return { ...describeArchivedSession(home, sessionId), lightweight: false };
  }
  const identity = checkpoint.identity ?? {};
  const titleInput = projectionValue(checkpoint, 'titleInput') ?? {};
  const first = titleInput.first;
  const titleValue = projectionValue(checkpoint, 'title');
  const listMeta = projectionValue(checkpoint, 'sessionListMetadata');
  const preset = projectionValue(checkpoint, 'agentPreset');
  const firstText = typeof first?.text === 'string' ? first.text.replace(/\s+/g, ' ').trim() : '';
  const title =
    typeof titleValue === 'string' && titleValue.trim() !== ''
      ? titleValue
      : firstText !== ''
        ? firstText.slice(0, 30)
        : '(untitled)';
  return {
    id: sessionId,
    title,
    ...
    createdAt: typeof identity.createdAt === 'number' ? identity.createdAt : undefined,
    // Prefer the artifact mtime (it tracks the last log write and can never be
    // stale); the checkpoint's last-prompt time is the fallback.
    lastActivity: known.mtimeMs ?? (typeof listMeta?.lastPromptAt === 'number' ? listMeta.lastPromptAt : undefined),
    // NOTE: deliberately NO userCount / assistantCount here. The checkpoint's
    // counters measure different things than `extractMeta` does -- its
    // `titleInput.count` is the number of prompts fed to title generation and
    // `turnOutline.turns` counts TURNS, while `extractMeta` counts
    // `user/message` and `assistant/message` RECORDS (one turn can hold many
    // assistant messages: measured 9 vs 135 on a real session). Showing them here
    // would silently relabel the numbers, so the counts arrive with the session's
    // detail view instead -- the panel fills a row in only after you open it.
    messageCountsKnown: false,
    missing: false,
    artifact: known.artifact,
    artifactBytes: known.bytes,
    lightweight: true,
  };
}
 ...
 *
 * @param options.lite - default `true`: build each row from the projection
 *   checkpoint + a stat instead of decoding its transcript. Measured here with
 *   9 archived sessions: ~300ms decoding everything vs ~10ms lite. Pass `false`
 *   to force the exact from-the-log metadata (diagnostics / tests).
 */
export function listArchivedSessions(home = resolveDshHome(), { lite = true } = {}) {
  const { archived, workspaces } = readWorkspaceStorage(home);
  const byPath = new Map(listWorkspaces(workspaces).map((w) => [w.path, w.title]));
  const artifacts = lite ? collectSessionArtifacts(home) : undefined;
  const sessions = archived.map((id) => {
    const entry = lite ? describeArchivedSessionLite(home, id, artifacts) : describeArchivedSession(home, id);
    return { ...entry, workspaceTitle: byPath.get(entry.cwd) ?? entry.cwd ?? '' };
  });
  return { count: sessions.length, sessions, lightweight: lite };
}
